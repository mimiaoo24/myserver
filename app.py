...
def init_student_table():
    ...
    conn.commit()
    conn.close()
    logging.info("Student table initialized.")
def init_student_table():
    conn = sqlite3.connect(DATABASE_NAME)
    c = conn.cursor()
    c.execute('''
    ...
    # Dữ liệu mẫu
    dummy_students = [
        ("001111111111", "Nguyễn Văn A", "12A1"),
        ("001111111112", "Trần Thị B", "12A1"),
        ("001111111113", "Lê Văn C", "12A2"),
        ("001111111114", "Phạm Thị D", "12A2"),
        ("001111111115", "Đỗ Văn E", "11B1")
    ]

    for cccd, name, classId in dummy_students:
        try:
            c.execute("INSERT INTO students (cccd, name, classId) VALUES (?, ?, ?)", (cccd, name, classId))
        except sqlite3.IntegrityError:
            continue  # bỏ qua nếu dữ liệu đã có

    conn.commit()
    conn.close()
    logging.info("Student table initialized with dummy data.")
...
def add_student():
    try:
        data = request.get_json()
        if not data or 'cccd' not in data or 'name' not in data or 'classId' not in data:
            return jsonify({'error': 'Thiếu thông tin học sinh (cccd, name, classId)'}), 400

        conn = sqlite3.connect(DATABASE_NAME)
        c = conn.cursor()
        c.execute('INSERT INTO students (cccd, name, classId) VALUES (?, ?, ?)',
                  (data['cccd'], data['name'], data['classId']))
        conn.commit()
        conn.close()

        return jsonify({'status': 'success', 'message': 'Đã thêm học sinh thành công'}), 201

    except sqlite3.IntegrityError:
        return jsonify({'error': 'Mã CCCD đã tồn tại'}), 409

    except Exception as e:
        logging.error(f"Lỗi thêm học sinh: {e}")
...
def reset_students():
    try:
        ...
        conn.commit()
        conn.close()
        return jsonify({'status': 'success', 'message': 'Đã xóa toàn bộ danh sách học sinh.'})
    except Exception as e:
        logging.error(f"Lỗi khi reset students: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
@app.route('/diem_danh', methods=['POST'])
def diem_danh():
    try:
        data = request.get_json()
        if not data or not all(k in data for k in ("cccd", "name", "classId")):
            return jsonify({'status': 'error', 'message': 'Thiếu thông tin CCCD, tên hoặc lớp'}), 400
        ...
        # Ghi vào bảng điểm danh
        conn = sqlite3.connect(DATABASE_NAME)
        c = conn.cursor()
        c.execute("INSERT INTO attendances (cccd, name, classId, timestamp) VALUES (?, ?, ?, ?)",
                  (cccd, name, classId, timestamp))
        conn.commit()
        conn.close()

        return jsonify({
            'status': 'success',
            'message': f'Điểm danh thành công cho {name}',
            'data': {
                'cccd': cccd,
                'name': name,
                'classId': classId,
        ...
        logging.error(f"Lỗi điểm danh: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
